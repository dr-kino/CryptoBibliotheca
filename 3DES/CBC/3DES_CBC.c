...
void desnbs(int codop, unsigned char *inp_c, unsigned char * chave,
		 unsigned char *scratch_buffer);
static void bits (unsigned char *INP,int INP_LEN,unsigned char *OUT,int OUT_LEN,
	      unsigned char *TABLE,int TABLE_LENGTH);
static void shift_left_bits (unsigned char *INP,int INP_LENGTH,int LOOP_NO,
...
static  void descompacta_decimaliza( unsigned char *origem, char *destino,
...
/* unsigned char * INP,
 ...
 int INP_LEN,
     ...
     TABLE_LENGTH;*/
 {
static unsigned char maskNBS[8] =
...
static  void compacta( char *origem, unsigned char *destino,
   int tamanho_da_origem )
{
...
{ int i;
 ...
 if (modo == 0) // criptografar
  ...
  { INSERIR(entrada,bloco_entrada,1,i+1,8);
    ...
    desnbs(0,vetor,chave1,saida);
    desnbs(1,saida,chave2,saida);
    desnbs(0,saida,chave3,saida);
   ...
   for (i=0; i < tam_entrada; i+=8)
   { INSERIR(entrada,bloco_entrada,1,i+1,8);
     desnbs(1,entrada,chave3,saida);
     desnbs(0,saida,  chave2,saida);
     desnbs(1,saida,  chave1,saida);
...
{ char bloco_entrada[17];
  ...
  printf("Bloco Entrada Exp  (256 bits ou 32 bytes)....: %s\n",bloco_entrada_expandido);
  printf("Chave cripto 1 (64 bits ou 8 bytes)..........: %s\n",chave1);
  printf("Chave cript 1 Expandida(128 bits ou 16 bytes): %s\n",chave1_expandida);
  printf("Chave cripto 2 (64 bits ou 8 bytes)..........: %s\n",chave2);
  printf("Chave cript 2 Expandida(128 bits ou 16 bytes): %s\n",chave2_expandida);  
  printf("Chave cripto 3 (64 bits ou 8 bytes)..........: %s\n",chave3);
  printf("Chave cript 3 Expandida(128 bits ou 16 bytes): %s\n",chave3_expandida);
  printf("Vetor Inicial (64 bits ou  8 bytes)..........: %s\n",vetor_inicial);
  printf("Vetor Inicial expandido(128 bits ou 16 bytes): %s\n",vetor_inicial_expandido);   
  printf("Bloco de Saida(128 bits ou 16 bytes).........: %s\n",bloco_saida);
  printf("Bloco Saida Expandido(256 bits ou 32 bytes)..: %s\n",bloco_saida_expandido);

  //decriptografar
  modo = 1;
  strcpy(bloco_entrada,"                ");
  strcpy(vetor_inicial,"inicio12");
